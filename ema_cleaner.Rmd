...
#for this script you will first need to download the EMAS from redcap and then download the DES_raw.csv file within the Andrew folder in Cablab outlook!

# Load DES PID lookup (the reason were calling this is because I actually couldnt figure out another way to organize by PID as opposed to record ID if needed you can go into my folder and download the des raw csv and just enter that file path for pid_path!)
...
# Main processing: loop over CSVs and compute intermediate data for final scores replace this with the file path to your EMA files exported from red cap! 
...
# Export final participant scores CSV this will create a seperate csv with the caluclated scores you will call this csv along with the EMAs in the app script! 
